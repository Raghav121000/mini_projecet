...
#can use try catch or if else also
def predict():
    ...
    if(x<=250):
        return render_template('co2.html',x='Value is : {}      '.format(*x)  +'    CAR IS ENVIRONMENT FRIENDLY')

    else:
        return render_template('co2.html',x='Value is : {}      '.format(*x)  +'    CAR IS NOT ENVIRONMENT FRIENDLY')
